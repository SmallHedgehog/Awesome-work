...
parser = argparse.ArgumentParser(description='ResNet32+CosineAnnealing+Mixup+LabelSmoothing+DecoupledWeightDecay')
parser.add_argument('--config', default='experiments/cifar10-bl_ca_mu_lb_dwdr/config.yaml')
...
def main():
    ...
    optimizer = SGD(model.parameters(), lr=config.lr_scheduler.base_lr, momentum=config.momentum,
            weight_decay=config.weight_decay, l2_reg=False)
