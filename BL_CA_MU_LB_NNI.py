...
from tricks.training_refinements.Label import LabelSmoothing
...
parser = argparse.ArgumentParser(description='ResNet32+CosineAnnealing+Mixup+LabelSmoothing with NNI')
parser.add_argument('--config', default='experiments/cifar10-bl_ca_mu_lb_nni/config.yaml')
...
def modify_config(rcv_config):
    try:
        config.lr_scheduler.base_lr = rcv_config['base_lr']
        config.alpha = rcv_config['alpha']
        config.label_smoothing = rcv_config['label_smoothing']
    except:
        pass
